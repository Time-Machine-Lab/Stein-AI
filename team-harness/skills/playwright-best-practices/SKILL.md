...
## Test-Engineer Boundary

When this skill is used by a test engineer, writing and fixing means working on Playwright tests, fixtures, test data, and test configuration. Inspect production code when needed, but do not modify it unless the user explicitly assigns a separate implementation task with suitable authority.

A failing test may reveal a product defect and can be a valid testing result. Before changing a test, distinguish product behavior, test defects, environment problems, and unresolved instability. Never weaken assertions, change expected behavior to match the implementation, skip checks, or add retries merely to make the suite pass. Preserve evidence and report confirmed product failures instead of continuing into production-code repair, merge, release, or team-workflow actions.
...
2. **If tests fail**:
   ...
   - Fix locators, waits, assertions, or fixtures only when evidence shows the test is defective
   - Preserve and report the failure when evidence indicates a product defect or remains inconclusive
   - Re-run tests after any test-side correction
3. **Conclude with evidence**: report passing, failing, or inconclusive status without manufacturing green
